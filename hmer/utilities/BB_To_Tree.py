...
class BBParser:
    ...
    def partition(self, snode_list, snode):
        ...
        for node in temp_list:
            if node[5] < snode[0]:  # node_centroidX < snode_minX
                if node[6] < snode[8]:  # node_centroidY < snode_sup # Topleft
                    ...
                    del_idx_list.append(idx)
            elif node[5] < snode[2]:  # node_centroidX < snode_maxX
                if node[6] < snode[8]:  # centroidy_node < snode_minY # Top # IMPORTANT FIX SHOULD COMPARE WITH SUP SUB THRESHOLD INSTEAD OF YMIN YMAX SINCE ABOVE AND BELOW ARE THE SAME OF YTHRESHOLD AS SUP AND SUB
                    snode[self.Atb['child_temp']][2].append(node[:])
                    del_idx_list.append(idx)
                elif node[6] > snode[7]:  # centroidy_node > snode_maxY # Bot # IMPORTANT FIX SHOULD COMPARE WITH SUP SUB THRESHOLD INSTEAD OF YMIN YMAX SINCE ABOVE AND BELOW ARE THE SAME OF YTHRESHOLD AS SUP AND SUB
